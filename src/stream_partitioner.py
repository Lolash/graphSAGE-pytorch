...
def partition_batches_from_queue(reading_queue: multiprocessing.Queue, writing_queue: multiprocessing.Queue, adj_list,
    ...
    while True:
        ...
        if node2idx is None:
            assignments, processing_time = _partition_one_batch(adj_list, edges_batch, bidirectional, features_batch, freqs, gap,
                                               graphsage,
                                               args.learn_method, args.num_classes, args.max_load,
                                               args.sorted_inference)
        else:
            assignments, processing_time = _partition_one_generated_batch(adj_list, edges_batch, bidirectional, features_batch, freqs,
                                                         gap, graphsage,
                                                         args.learn_method, args.num_classes, args.max_load,
...
def _partition_one_generated_batch(adj_list, edges_batch, bidirectional, features_batch, freqs, gap, graphsage,
                                   learn_method,
    ...
    for e in edges_batch:
        if node2idx[e[0]] not in adj_list:
            adj_list[node2idx[e[0]]].add(node2idx[e[1]])
            added_edges.add((node2idx[e[0]], node2idx[e[1]]))
        if bidirectional and node2idx[e[1]] not in adj_list[node2idx[e[0]]]:
            adj_list[node2idx[e[1]]].add(node2idx[e[0]])
            added_edges.add((node2idx[e[1]], node2idx[e[0]]))
    with torch.no_grad():
        if learn_method == "sup_edge":
            ...
            srcs2idx = [item for item in map(lambda e: node2idx[e[0]], edges_batch)]
            dsts2idx = [item for item in map(lambda e: node2idx[e[1]], edges_batch)]
            ...
            pairs = [item for sublist in map(lambda e: [e[0], e[1]], edges_batch) for item in
                     sublist]
            pairs2idx = [item for sublist in map(lambda e: [node2idx[e[0]], node2idx[e[1]]], edges_batch) for item in
                         sublist]
...
def _partition_one_batch(adj_list, edges_batch, bidirectional, features, freqs, gap, graphsage, learn_method,
    ...
    with torch.no_grad():
        if learn_method == "sup_edge":
            srcs = [item for item in map(lambda e: e[0], edges_batch)]
            dsts = [item for item in map(lambda e: e[1], edges_batch)]
            srcs_embs = graphsage(srcs, features, adj_list)
            dsts_embs = graphsage(dsts, features, adj_list)
            embs = torch.cat([srcs_embs, dsts_embs], 1)
            predicts = gap(embs)
            _, max_idx = torch.max(predicts, dim=1)
            max_idx = max_idx.squeeze().tolist()
